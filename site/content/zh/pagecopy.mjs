...
export const COPY = {
  ...
  home: {
    ...
    h1: `从零开始<br>学习智能体安全。`,
    lede1: `给语言模型接入工具，会改变它面对的安全问题。智能体读取网页、邮件或文档时，会把不可信文本
  与控制行动的指令放在同一个上下文里，而且往往还持有凭据或敏感数据。本课程讨论由此产生的攻击、可用的
  防御，以及这些防御在面对知情攻击者时的边界。`,
    lede2: `每一章都包含一张机制图、一个浏览器内实验、一个自包含的 Python 示例、六道评分题，以及相关
  研究的参考文献。`,
    btnStart: `从 A01 开始 →`,
   ...
   每一章防御其中一个。虚线是唯一真正重要的那条边界，而模型站在它错误的那一侧。`,
    ...
    claimsH2: `本课程的核心判断`,
    claim1t: `提示注入本质上是权限隔离问题。`,
    claim1b: `当不可信内容和指令共享上下文时，模型行为本身无法提供可靠的安全边界。架构决定一次成功的注入
  能够触及什么。`,
    claim2t: `检测有用，但不等于遏制。`,
    claim2b: `护栏可以提高攻击成本；能力范围限定、信息流控制和出站策略还可以限制攻击造成的损害。`,
    claim3t: `许多控制措施都是成熟的安全工程。`,
    claim3b: `最小权限、沙箱、供应链审查、出站控制和审计日志依然重要。模型带来了新的接口，但系统仍然需要
  传统的安全控制。`,
...
不需要安全背景，也不需要任何机器学习理论。所有东西都跑在仓库自带的一个确定性桩模型上。`,
  ...
  修复面前失手。`,
    ...
    currH2: `课程大纲`,
    currP: `六个部分建议按顺序阅读：从威胁模型出发，经过攻击和控制，最后进入评估与运营。每个部分以一个
项目收尾，毕业项目把这些内容串联起来。`,
    part: (id) => `第 ${id} 部分`,
    chLines: (n) => `${n} 行`,
    finalProject: `毕业项目`,
...
<code>quiz</code> 和 <code>refs</code>。`,
  ...
  answers: {
    title: `练习答案`,
    description: `课程中每一道练习的参考答案，按章节排列，给出推理过程和用来验证的检查方法。`,
    kicker: `参考`,
    h1: `练习答案`,
    sub: (n) => `${n} 道题的完整解答，每章一节。`,
    intro: `每一章结尾都有三道需要动手的题。答案统一放在这里，而不是直接跟在题目下面，
这样读到章节末尾时不会被自己的题目剧透。每条答案给出的是推理过程，以及用来确认真的做对了的检查方法，
而不只是结论。如果这个问题本来就没法彻底解决，答案里会直说。`,
    jump: `跳转到某一部分`,
  },
  cheatsheet: {
    title: `速查表`,
    description: `把整门课压缩到一页：六条贯穿始终的主线、27 章的论点与对策、六个威胁面，以及真正能把损失兜住的控制措施。`,
    kicker: `参考`,
    h1: `速查表`,
    sub: (n) => `六条主线、${n} 章正文，以及真正配得上"控制措施"这个名字的那些。`,
    intro: `这一页是设计评审之前用来重读的，不是用来入门的。每一行都压缩了一整章的论证，点一下就能回到原章。
如果某一行读上去像废话，那一章正好讲的就是它为什么不是废话。`,

    spineH2: `六条主线`,
    spineSub: `别的都可以忘，这六条留下。`,

    chaptersH2: `每一章的论点与对策`,
    chapterCols: [`章节`, `这一章确立了什么`, `你该怎么办`],

    surfacesH2: `六个威胁面`,
    surfaceSub: (n) => `${n} 类漏洞，按进入位置归组。`,
    surfaceCols: [`&nbsp;`, `威胁面`, `什么会落在这里`, `漏洞类别`],

    controlsH2: `能把损失兜住的控制措施`,
    controlsSub: (bound, total) => `防御地图里的 ${total} 项控制措施中，有 ${bound}
项在模型已被完全攻陷时仍然成立。其余的只是抬高攻击者的成本，这钱值得花，但它和"兜住"不是一回事。`,
    controlCols: [`控制措施`, `作用`, `章节`],
    controlsMore: (raise, support, link) => `另外 ${raise} 项抬高成本的措施和 ${support}
项支撑性措施，都列在<a href="${link}">防御地图</a>上。`,

    outroH2: `接下来看什么`,
    outro: (answers, curriculum, glossary) => `把练习做一遍，再对照<a href="${answers}">参考答案</a>检查。
完整的论证在<a href="${curriculum}">课程大纲</a>里。想查某个词就翻<a href="${glossary}">术语表</a>。`,
  },
};
