...
export const TERMS = [
  ...
  ['Tool poisoning', 'attack', `Malicious instructions placed in tool metadata — name, description,
    parameter docs — which reach the model before the tool is ever invoked.`, 'a11'],
  ...
  ['Plan-then-execute', 'defense', `The full action plan is fixed before any untrusted content is
  ...
  ['Taint tracking', 'defense', `The practical form of IFC: mark data from untrusted sources and
    propagate the mark through every derived value.`, 'a21'],
  ['Attenuated token', 'defense', `A credential deliberately narrowed — by scope, audience, resource
    and lifetime — before being handed to an agent, so a stolen token is worth less.`, 'a22'],
  ...
  ['Reversibility class', 'defense', `Grading actions by how hard they are to undo — reversible, costly,
